            ...
            <div class="container">
      		...
      		<div class="mws-panel grid_8">
                	<div class="mws-panel-header">
                    	<span class="mws-i-24 i-table-1" style="display: block; float: left;">所有用户列表</span>
                    	<div id="templatemo_search">
			                <form action="<?php echo site_url("admin/back/get_search_user");?>" method="post">
			                	...
			                	<input type="submit" name="submit" value="查询"  id="searchbutton" title="Search" class="sub_btn" />
			                  	<input type="text" placeholder="查询用户" name="search" class="txt_field" />
			                </form>
			            </div>
                    </div>
                    <div id="BlogOpts" style="background:#eee;border:1px solid #ccc;margin:5px 0 10px 0;padding:3px 10px;color:#999;">
                    	<a href="<?php echo site_url("admin/back/add_user_view")?>" style="text-decoration: none;"><img src="<?php echo site_url('assets/admin/images/add.png');?>" style="width: 15px; height: 15px;">添加</a>&nbsp;|
                    ...
                    <div class="mws-panel-body">
                        <table class="mws-table">
                            <thead>
                             ...
                             <tbody>
                             	<?php 
                             		foreach ($users as $t_user) {
                             	?>    
	                              <tr class="gradeX">
	                                 <td><input name="id" value="<?php echo $t_user->user_id;?>" type="checkbox"></td>
	                                 <td><?php echo "$t_user->user_name"?></td>
	                                 <td><?php echo "$t_user->passwd"?></td>
	                                 <td><?php echo "$t_user->tel"?></td>
	                                 <td><?php echo "$t_user->email"?></td>
	                                 <td><?php echo "$t_user->address"?></td>
	                                 <td><a href="<?php echo site_url("admin/back/edit_user_view/".$t_user->user_id)?>" style="text-decoration: none;"><img src="<?php echo site_url('assets/admin/images/edit.png');?>" style="width: 15px; height: 15px;">编辑</a>&nbsp;|
	                                 	<a href="<?php echo site_url("admin/back/delete_user/".$t_user->user_id)?>" style="text-decoration: none;"><img src="<?php echo site_url('assets/admin/images/delete.png');?>" style="width: 15px; height: 15px;">删除</a>&nbsp;
	                                 </td>
	                              </tr>
                                <?php } ?>
                            </tbody>
    ...
    <script type="text/javascript">
		...
		function delete_sel(){
			...
			if(ids.length > 0){
				...
				$.get("<?php echo site_url("admin/back/remove_user")?>","id="+ids,function(html){
					location.reload();
				});
			}
			else {
